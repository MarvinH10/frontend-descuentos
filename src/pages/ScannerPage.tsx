...
const ScannerPage: React.FC = () => {
  ...
  const searchSingleProduct = useCallback(async (code: string) => {
    ...
    // Actualizar estado del item a 'searching'
    setScannedItems(prev => 
      prev.map(item => 
        item.code === code 
    ...
    try {
      ...
      // Actualizar estado del item a 'found'
      setScannedItems(prev => 
        prev.map(item => 
          item.code === code 
            ? { ...item, status: 'found' } 
      ...
      // Actualizar estado del item a 'error'
      setScannedItems(prev => 
        prev.map(item => 
          item.code === code 
            ? { 
  ...
  // Buscar todos los productos pendientes
  ...
  // Actualizar precio cuando cambia el producto o cantidad
  useEffect(() => {
    if (product) {
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-8">
          ...
          {/* Panel derecho: Lista de códigos escaneados */}
          <div>
            <ScannedItemsList
              items={scannedItems}
